...
import { getTerms } from "@/content/legal/loaders";
import {
...
export async function generateMetadata({
  ...
  return generateLegalMetadata({ locale, slug: "terms" });
}

export default async function TermsPage({ params }: PageProps) {
  const { locale } = await params;
  ensureLegalLocale(locale, "terms");

  const contentLocale: LegalContentLocale = isLegalContentLocale(locale)
    ? locale
    : "en";
  const document = getTerms(contentLocale);
