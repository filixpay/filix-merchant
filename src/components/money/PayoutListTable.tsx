...
import { ApiError, type MoneyPayoutView } from "@/lib/api";
...
export interface PayoutListTableProps {
  items: MoneyPayoutView[];
  loading: boolean;
  ...
  onRowClick?: (payoutId: string) => void;
  detailHref?: (payoutId: string) => string;
}
...
export default function PayoutListTable({
  items,
  loading,
  isRefreshing = false,
  error = null,
  ...
  const columns: ColumnsType<MoneyPayoutView> = [
    {
      title: t("columns.id"),
      dataIndex: "payoutId",
      key: "payoutId",
  ...
  return (
    <>
      ...
      <Table
        ...
        scroll={{ x: 800 }}
        onRow={(row) => ({
          onClick: () => onRowClick?.(row.payoutId),
