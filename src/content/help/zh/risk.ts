...
export const zhRisk: Record<string, HelpArticleContent> = {
  "risk/refunds": {
    ...
    body: {
      whoFor: "处理客户退款、配置自动执行阈值，或审批待处理退款的运营人员。",
      whenToUse:
        "当你打开「交易 → 退款」「退款设置」「退款审批」，或从订单发起退款后，使用本指南。",
      beforeYouStart: [
        "准备好原订单号或退款单号。",
        "确认角色可创建退款、修改退款设置，或处理退款审批。",
        "了解是否启用按金额审批，以及自动执行阈值是多少。",
      ],
      blocks: [
        {
          type: "paragraph",
        ...
        {
          type: "paragraph",
          text: "退款设置决定退款是自动执行，还是需商户审批。结算币种只读展示。可开启「跳过按金额审批」，或设置结算币种下的「自动执行阈值」。跨币种退款会先经 FX 换算再比较；若无报价，退款仍会创建但需等待审批。保存设置或刷新以重新加载。",
        },
        {
          type: "fields",
          rows: [
            {
              field: "结算币种",
              description: "只读；用于将退款金额与阈值比较的币种。",
            },
            {
              field: "跳过按金额审批",
              description:
                "开启后，仅金额不会触发审批。FX 换算失败仍可能需要审批。",
            },
            {
              field: "自动执行阈值",
              description:
                "不超过该金额（结算币种）的退款自动执行；更大金额将待审批。未设置商户覆盖时可能显示平台默认阈值提示。",
            },
          ],
        },
        {
        ...
        {
          type: "paragraph",
          text: "退款审批列出超过自动执行阈值、等待审批的退款。可查看退款单号、订单号、金额、状态与创建时间。批准后将通过支付通道执行退款；拒绝会取消该请求，不会打到通道。可打开查看详情阅读完整退款记录。",
        },
        {
          type: "heading",
          text: "关键字段与状态",
